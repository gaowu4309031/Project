...
//	osal_start_reload_timer(UserBase1_TaskID, USER_TASK1_EVENT0, 3000);
}
...
//	osal_start_reload_timer(UserBase3_TaskID, USER_TASK3_EVENT0, 1000);
}
...
void UserBase4_Init( uint8 task_id )
{
	...
	osal_start_reload_timer(UserBase4_TaskID, USER_TASK4_EVENT0, 200);
}
...
bStatus_t User_NotifyData(uint8_t *notiData, uint8_t notiLen)
{
    ...
    noti.handle = UserProfileAttrTbl[4].handle;
    noti.len = notiLen;
