...
int hal_uart_send_byte_to_ascii(uint16_t data)
{
	uint8_t temp[6];
	...
	temp[0] = (data % 100000 / 10000) + '0';
   
    return hal_uart_send_buff(UART0, temp, 6);				
}

void hal_uart_send_buff_simple(char *data)
{
	char *p_tmep = data;
	while(*p_tmep)
	{
		hal_uart_send_byte(UART0, *p_tmep);
		p_tmep++;
	}
}
