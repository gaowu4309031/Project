...
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL    24//13//24//32//80

// Maximum connection interval (units of 1.25ms, 800=1000ms) if automatic parameter update request is enabled
#define DEFAULT_DESIRED_MAX_CONN_INTERVAL     800//40//800//48//800

// Slave latency to use if automatic parameter update request is enabled
#define DEFAULT_DESIRED_SLAVE_LATENCY         0

// Supervision timeout value (units of 10ms, 1000=10s) if automatic parameter update request is enabled
#define DEFAULT_DESIRED_CONN_TIMEOUT          500//400//500//1000
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "YK01-FFFF";
/*********************************************************************
    LOCAL FUNCTIONS
*/
...
{
    simpleBLEPeripheralPasscodeCB,                     // Passcode callback (not used by application)
...
//void uart_input_evt(uart_Evt_t* evt)
//{
//}
/*********************************************************************
    PUBLIC FUNCTIONS
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
    ...
    {
        uint16 advInt = 800;//3200;//800;//2400;//1600;//1600;//800;//1600;   // actual time = advInt * 625us
...
//	UserProfile_AddService(GATT_ALL_SERVICES);
...
/*********************************************************************
    ...
    @return  none
*/
extern void Ble_Status_Clear(void);
static void peripheralStateNotificationCB( gaprole_States_t newState )
{
    switch ( newState )
    {
    case GAPROLE_STARTED:
    {
        ...
        osal_memcpy(&scanRspData[7],&str_addr[2],12);
        osal_memcpy(&attDeviceName[5],&str_addr[2],12);
        GAPRole_SetParameter( GAPROLE_SCAN_RSP_DATA, sizeof ( scanRspData ), scanRspData );
        // Set the GAP Characteristics
        GGS_SetParameter( GGS_DEVICE_NAME_ATT, GAP_DEVICE_NAME_LEN, attDeviceName );
        GAPRole_SetParameter( GAPROLE_ADVERT_ENABLED, sizeof( uint8 ), &initial_advertising_enable );
...
static void simpleBLEPeripheralPasscodeCB( uint8* deviceAddr, uint16 connectionHandle,
                                        uint8 uiInputs, uint8 uiOutputs )
{
    MY_LOG("simpleBLEPeripheralPasscodeCB ,connectionHandle is %d\r\n",connectionHandle);
...
static void simpleBLEPeripheralPairStateCB( uint16 connHandle, uint8 state, uint8 status )
{
    MY_LOG("simpleBLEPeripheralPairStateCB in param state 0x%02X,status 0x%02X\r\n",state,status);

    if ( state == GAPBOND_PAIRING_STATE_STARTED )
    {
        MY_LOG( "Pairing started\n" );
    }
    else if ( state == GAPBOND_PAIRING_STATE_COMPLETE )
    {
        if ( status == SUCCESS )
        {
            MY_LOG( "Pairing success\n" );
        }
        else
        {
            MY_LOG( "Pairing fail\n" );
