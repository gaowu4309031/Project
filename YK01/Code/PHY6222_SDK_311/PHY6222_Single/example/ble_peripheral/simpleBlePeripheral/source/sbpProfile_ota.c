...
static bStatus_t simpleProfile_WriteAttrCB( uint16 connHandle, gattAttribute_t* pAttr,
                                            uint8* pValue, uint16 len, uint16 offset )
{
	extern uint8_t BleTransmissionBuff[BLE_DATA_LEN];
	extern uint8_t BleTransmissionLen;
	extern uint8_t BleBuffProcessFlag;					//蓝牙数据处理标志
	extern uint8_t BleTurnBuff[BLE_DATA_LEN];
	
    bStatus_t status = SUCCESS;
    uint8 notifyApp = 0xFF;
	
	LOG("simpleProfile_WriteAttrCB \n");
...
//            }
			...
			{
				uint8* pCurValue = BleTransmissionBuff;
